...
class UpcomingDataSource(val apiService: ApiService) : PagingSource<Int, com.example.movieappwithpaging.models.upcomingModel.Result>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, com.example.movieappwithpaging.models.upcomingModel.Result> {
        try {
            //1-page ni ovoladi starting page (position) sifatida
            val nextPageNumber = params.key ?: 1

            //linkga pageNumber ni berib yuboradi
            val passengers = apiService.getUpcoming("b8e4ead2a823c6e5c43ea97765f8c0d5", nextPageNumber)
    ...
    override fun getRefreshKey(state: PagingState<Int, com.example.movieappwithpaging.models.upcomingModel.Result>): Int? {
